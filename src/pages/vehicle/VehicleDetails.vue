<template>
  <v-container>
    <v-row
      v-if="
        vehicle.status === 'NOT AVAILABLE' || vehicle.status === 'AVAILABLE'
      "
      justify="center"
      class="mb-8"
    >
...
<script>
  ...
  export default {
    ...
    methods: {
      async changeStatus() {
        try {
          let res = await this.$store.dispatch("vehicle/changeStatus", {
    ...
    async beforeMount() {
      try {
        const res = await this.$store.dispatch("vehicle/getSingleVehicle", {
          vehicleId: this.$route.params.id,
        });
        this.reactive.vehicle = res.data;
        if (this.reactive.vehicle.images.length > 0) {
          this.reactive.vehicle.images = this.reactive.vehicle.images.map(
            (image) => image.url
          );
        } else {
          this.reactive.vehicle.images[0] =
            "https://dynaimage.cdn.cnn.com/cnn/c_fill,g_auto,w_1200,h_675,ar_16:9/https%3A%2F%2Fcdn.cnn.com%2Fcnnnext%2Fdam%2Fassets%2F210528125649-rolls-royce-boat-tail.jpg";
        }

        if (
          this.reactive.vehicle.documents &&
          this.reactive.vehicle.documents.length
        ) {
          this.motDoc = this.reactive.vehicle.documents.find(
            (x) => x.type === "MOTL"
          );
          this.taxDoc = this.reactive.vehicle.documents.find(
            (x) => x.type === "TAXL"
          );
          this.vehicelLogDoc = this.reactive.vehicle.documents.find(
            (x) => x.type === "VEHICLELOG"
          );
        }

        this.category = Object.values(vehicleCategories).filter(
          (x) => x.status === this.vehicle.status
        )[0];
      } catch (e) {
